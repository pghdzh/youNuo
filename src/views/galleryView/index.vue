...
$bg: #050714; // 深海夜色（卡提希娅风）
$accent-1: #4ea6ff; // 暗紫主光（冷雅）
$accent-2: #8fd9ff; // 冷海蓝高光（湿光感）
$text: #f4f8fb; // 主文字（微冷象牙）
@use "sass:color";

$venom: #8ca6bd;
$muted: rgba(234, 233, 238, 0.78);
...
.gallery-container {
  ...
  /* 右侧排行面板：卡提希娅风格（清冷冰蓝 + 圣光金 + 深海暗蓝），已写实颜色（无变量）*/
  .ranking-panel {
    width: 220px;
    padding: 16px;
    position: fixed;
    ...
    .panel-header {
      ...
      .ranking-title {
        ...
        font-size: 1.05rem;
        font-weight: 900;
        color: rgba(234, 247, 255, 0.92); /* 稍微柔和的冷白 */
    ...
    .ranking-item {
      ...
      /* === 尤诺风格名次卡 === */
      &.rank-1 {
        position: relative; /* 便于伪元素定位 */
        /* 主体：冷月蓝到银白的渐变，末端带极轻金色点缀 */
        background: linear-gradient(
          135deg,
          #9fe0ff 0%,
          rgba(94, 179, 255, 0.92) 28%,
          rgba(210, 242, 255, 0.72) 68%,
          rgba(255, 215, 138, 0.06) 100%
        );
        color: #041018; /* 深色文字，保证可读性 */

        border-radius: 10px;
        box-shadow:
    /* 月光主晕 */ 0 18px 56px rgba(78, 160, 255, 0.14),
          /* 轻微金色边缘光（点缀） */ 0 0 28px rgba(255, 216, 134, 0.06),
          /* 内部微弱冷光 */ inset 0 1px 0 rgba(255, 255, 255, 0.06);
        .rank,
        .name,
        .count {
          color: #041018;
          text-shadow: 0 2px 6px rgba(78, 160, 255, 0.04);
        }
      }

      /* 亚军：更偏夜色，保留月光高光（冷银为主），文字为近白 */
      &.rank-2 {
        position: relative;
        background: linear-gradient(
          135deg,
          #3b5f98 0%,
          rgba(78, 160, 255, 0.92) 42%,
          rgba(18, 34, 58, 0.95) 100%
        );
        color: #eaf7ff; /* 冷白文字 */

        border-radius: 10px;
        box-shadow: 0 14px 48px rgba(18, 34, 58, 0.12),
          0 6px 28px rgba(78, 160, 255, 0.06) inset;
      ...
      /* 季军：更沉稳的深海基调，保留月色冷高光并用极细金色作点缀 */
      &.rank-3 {
        position: relative;
        background: linear-gradient(
          135deg,
          #2f57a8 0%,
          rgba(65, 159, 222, 0.88) 56%,
          rgba(6, 18, 36, 0.96) 100%
        );
        color: #eaf7ff;

        border-radius: 10px;
        box-shadow: 0 10px 36px rgba(16, 26, 44, 0.1),
          0 0 20px rgba(78, 160, 255, 0.04) inset;

        .rank {
          color: rgba(255, 235, 200, 0.95); /* 近金暖点缀，微弱 */
